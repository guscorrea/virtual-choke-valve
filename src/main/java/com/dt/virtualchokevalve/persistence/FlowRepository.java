...
public class FlowRepository {
	...
	private Mapper<Flow> mapper;

	private Session session;

	private static final String TABLE = "flow";

	public FlowRepository(MappingManager mappingManager) {
		createTable(mappingManager.getSession());
		this.mapper = mappingManager.mapper(Flow.class);
	...
	public List<Flow> findAllByChokeValveId(UUID chokeValveId) {
		final ResultSet result = session.execute(select().all().from(TABLE).where(eq("choke_valve_id", chokeValveId)));
		return mapper.map(result).all();
	}

	public List<Flow> findAllByChokeValveIdAndDateTime(UUID chokeValveId, String startDateTime, String endDateTime) {
