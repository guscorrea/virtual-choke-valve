...
public class CustomMeasureRepository {
	...
	public List<CustomMeasure> findAllByChokeValveId(UUID chokeValveId) {
		final ResultSet result = session.execute(select().all().from(TABLE).where(eq("choke_valve_id", chokeValveId)));
		return mapper.map(result).all();
	}

	public List<CustomMeasure> findAllByChokeValveIdAndType(UUID chokeValveId, String propertyType) {
		final ResultSet result = session.execute(
				select().all().from(TABLE)
						.where(eq("choke_valve_id", chokeValveId))
						.and(eq("property_type", propertyType)));
		return mapper.map(result).all();
	}

	public List<CustomMeasure> findAllByChokeValveIdAndTypeAndDateTime(UUID chokeValveId, String propertyType, String startDateTime,
			String endDateTime) {
		final ResultSet result = session.execute(
				select().all().from(TABLE)
						.where(eq("choke_valve_id", chokeValveId))
